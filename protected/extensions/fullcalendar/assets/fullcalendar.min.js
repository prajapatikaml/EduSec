...
"<td class='fc-"+za[C.getDay()]+" "+R+"-state-default fc-day"+(k*Q+q)+(q==ka?" fc-leftmost":"")+(Z>1&&C.getMonth()!=v?" fc-other-month":"")+(+C==+x?" fc-today "+R+"-state-highlight":" fc-not-today")+"'>"+(g?"<div class='fc-day-number id='><a href='hrms/nationalHolidays/addholiday?date="+C.getDate()+"-"+(C.getMonth()+1)+"-"+(1900+C.getYear())+"' target ='_self' >"+C.getDate()+"</div>":"")+"<div class='fc-day-content'><div style='position:relative'>&nbsp;</div></div></td>";D(C,1);ga&&ba(C)}n+="</tr>"}u=m(n+"</tbody>").appendTo(M);r(u.find("td"));sa=m("<div style='position:absolute;z-index:8;top:0;left:0'/>").appendTo(a)}}function h(n){ua=
